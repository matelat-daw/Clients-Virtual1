...
class App {
    constructor() {
        ...
        this.routes = {
            ...
            'customer-details': this.loadCustomers
        };
    ...
    static init() {
        ...
        // 1. Configurar rutas primero (esto pinta el Home inmediatamente)
        instance.setupRouting();
        
        // 2. Cargar Navbar y Sesión de forma asíncrona pero sin delay artificial
        instance.loadNavbar().catch(err => console.warn('Navbar diferido:', err));
    }
    ...
    navigateTo(path) {
        ...
        try {
            ...
            // Efecto de fade-out suave antes de cambiar
            const outlet = document.getElementById('router-outlet');
            if (outlet) {
                outlet.style.opacity = '0.5';
                outlet.style.transition = 'opacity 0.15s ease-in-out';
            }

            setTimeout(() => {
                this.handleRouteChange();
                if (outlet) outlet.style.opacity = '1';
            }, 150);
        } catch (error) {
    ...
    handleRouteChange() {
        ...
        // El Navbar se actualiza después para no bloquear el renderizado inicial
...
const outlet = document.getElementById('router-outlet');
    ...
    loadCustomers(customerId) {
        ...
        // Si no hay customerId por parámetro de ruta, intentar obtenerlo de la URL (?id=X)
        if (!customerId) {
