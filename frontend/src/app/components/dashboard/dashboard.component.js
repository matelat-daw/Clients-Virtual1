...
Utils.showMessage('Error', 'No se pudo cargar el dashboard', 'error');
    ...
    async loadUserData() {
        ...
        // Intentar refrescar los datos desde la API con cache
        try {
            const url = API_CONFIG.BASE_URL + API_CONFIG.ENDPOINTS.PROFILE;
            // Usar cache para el perfil (válido por 2 min) para evitar peticiones repetitivas
        ...
        // Actualizar foto de perfil - SOLO imagen del usuario, NUNCA fallback
        ...
        if (profilePictureImg) {
            ...
            setTimeout(() => {
                ...
                if (photoUrl) {
                    // Usuario tiene foto personalizada - cargar CON versionado contra cache
                    const version = window.__profileImageVersion || Date.now();
                    const sep = photoUrl.includes('?') ? '&' : '?';
                    const finalUrl = `${photoUrl}${sep}v=${version}`;
                    
                    console.log('🖼️ Cargando foto en dashboard:', finalUrl);
                    profilePictureImg.src = finalUrl;
                    profilePictureImg.style.display = 'block';
                    profilePictureImg.style.visibility = 'visible';
                    
                    // Si la imagen falla - OCULTARLA, NO cargar fallback
                    profilePictureImg.onerror = () => {
                        console.error('Error cargando foto de perfil en dashboard');
                        profilePictureImg.src = '';
                        profilePictureImg.style.display = 'none';
                        profilePictureImg.style.visibility = 'hidden';
                        profilePictureImg.onerror = null;
                    };
                } else {
                    // Sin foto personalizada - OCULTAR, NO MOSTRAR FALLBACK
                    console.log('ℹ️ Usuario sin foto personalizada en dashboard');
                    profilePictureImg.src = '';
                    profilePictureImg.style.display = 'none';
                    profilePictureImg.style.visibility = 'hidden';
                    profilePictureImg.onerror = null;
