...
class NavbarComponent {
    ...
    async reinit() {
        try {
            console.log('🔄 NavBar.reinit() iniciado');
            // No recargar el HTML, solo actualizar el estado de autenticación
            this.updateAuthStatus();
            this.loadUserData();
            console.log('✅ NavBar.reinit() completado');
        } catch (error) {
            console.error('❌ Error en NavBar.reinit():', error);
        }
    }
    ...
    async init() {
        try {
            // Usar cache para el HTML del navbar si ya se cargó una vez
            ...
            if (container) {
                ...
                // Ejecutar actualización de estado inmediatamente si es posible
                this.updateAuthStatus();
...
this.showUnauthenticatedView();
    ...
    loadUserData() {
        try {
            console.log('🔄 NavBar.loadUserData() iniciado');
            
            // Validar que AuthService esté definido
            if (typeof AuthService === 'undefined') {
                console.warn('⚠️ AuthService no definido');
                return;
            }
            ...
            // Actualizar foto de perfil - CRÍTICO: SOLO imagen del usuario, NUNCA por defecto
            ...
            if (profilePic) {
                ...
                setTimeout(() => {
                    ...
                    if (imageUrl) {
                        ...
                        console.log('🖼️ Cargando avatar en navbar:', finalUrl);
                        
                        // Validar que la imagen realmente se cargó
                        profilePic.onload = function() {
                            console.log('✅ Avatar en navbar cargado exitosamente');
                            this.style.display = 'block';
                            this.style.visibility = 'visible';
                        };
                        
                        profilePic.onerror = function() {
                            console.error('❌ Error al cargar avatar en navbar:', finalUrl);
                            this.src = '';
                        ...
                        // Sin foto - OCULTAR TOTALMENTE
                        console.log('ℹ️ Usuario sin foto de perfil en navbar');
                        profilePic.src = '';
                ...
                console.warn('⚠️ Elemento navbarProfilePic no encontrado');
            }
            ...
            console.log('✅ NavBar.loadUserData() completado');
        } catch (error) {
            console.error('❌ Error en loadUserData del navbar:', error);
        }
    }
