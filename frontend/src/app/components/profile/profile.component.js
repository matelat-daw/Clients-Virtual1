...
class ProfileComponent {
    ...
    async init() {
        try {
            ...
            console.error('❌ Error inicializando ProfileComponent:', error);
    ...
    async loadUserData(forceRefresh = false) {
        try {
            ...
            if (!user) {
                console.warn('⚠️ No hay usuario en sesión, intentando recuperar...');
                // Intentar recuperar desde sessionStorage directamente
                const stored = sessionStorage.getItem('user_session');
                if (stored) {
                    try {
                        user = JSON.parse(stored);
                        console.log('✅ Sesión recuperada desde sessionStorage');
                    } catch (error) {
            ...
            if (forceRefresh) {
                try {
                    ...
                    console.warn('No se pudo refrescar el perfil desde API:', error);
                }
            }

            // Actualizar foto de perfil - CRÍTICO: Cargar SIEMPRE desde la sesión del usuario
            // NUNCA usar imagen por defecto, SOLO imagen del usuario de su carpeta ID
            console.log('📸 Cargando foto de perfil del usuario...');
            
            // Usar setTimeout para asegurar que el elemento está en el DOM
            setTimeout(() => {
                const profilePic = document.getElementById('profilePicturePreview');
                if (profilePic) {
                    // Obtener URL de la sesión del usuario
                    const imageUrl = AuthService.getProfilePictureUrl();
                    
                    if (imageUrl) {
                        // Usuario tiene foto personalizada - cargar SIEMPRE
                        const version = window.__profileImageVersion || Date.now();
                        const sep = imageUrl.includes('?') ? '&' : '?';
                        const finalUrl = `${imageUrl}${sep}v=${version}`;
                        
                        console.log('✅ Foto de perfil del usuario encontrada:', finalUrl);
                        
                        // Validar que la imagen realmente se cargó
                        profilePic.onload = function() {
                            console.log('✅ Imagen cargada exitosamente en profile');
                            this.style.display = 'block';
                            this.style.visibility = 'visible';
                        };
                        
                        profilePic.onerror = function() {
                            console.error('❌ Error al cargar imagen:', finalUrl);
                            this.src = '';
                        ...
                        // Sin foto personalizada - OCULTAR TOTALMENTE
                        console.log('ℹ️ Usuario sin foto personalizada');
                        profilePic.src = '';
                        profilePic.style.display = 'none';
                        profilePic.style.visibility = 'hidden';
                        profilePic.onerror = null;
                        profilePic.onload = null;
                    }
                }
            }, 50);
    ...
    async handleUploadPicture(e) {
        ...
        try {
            ...
            console.log('✅ Respuesta inicial del POST /picture:', updatedProfile);
            ...
            try {
                ...
                if (profileResponse.ok) {
                    ...
                    if (profileData.success && profileData.data) {
                        ...
                        AuthService.setUserSession(updatedProfile);
                        console.log('✅ Datos refrescados desde API:', updatedProfile);
                    } else {
                        console.warn('⚠️ GET /profile no devolvió datos válidos');
                    }
                } else {
                    console.warn(`⚠️ GET /profile devolvió status ${profileResponse.status}`);
                }
            } catch (refreshErr) {
                console.warn('⚠️ Error refrescando desde API:', refreshErr.message);
            ...
            console.log('🔍 Verificando disponibilidad de imagen en API...');
            const imageUrl = AuthService.getProfilePictureUrl();
            ...
            if (imageUrl) {
                ...
                if (!version) {
                    console.warn('⚠️ Versión no establecida');
                    window.__profileImageVersion = Date.now();
                }
                ...
                while (!imageReady && attempts < maxAttempts) {
                    try {
                        console.log(`⏳ Intentando verificar imagen (${attempts + 1}/${maxAttempts})...`);
                        const imgResponse = await fetch(testUrl);
                        
                        if (imgResponse.status === 200 || imgResponse.status === 304) {
                            console.log('✅ Imagen confirmada en API:', testUrl);
                            imageReady = true;
                            break;
                        } else {
                            console.log(`⏳ Status ${imgResponse.status}, reintentando...`);
                            attempts++;
                            await new Promise(resolve => setTimeout(resolve, 300));
                        }
                    } catch (err) {
                        console.log(`⏳ Error verificando (${attempts + 1}/${maxAttempts}):`, err.message);
                        attempts++;
                        await new Promise(resolve => setTimeout(resolve, 300));
                    }
                }
                ...
                if (imageReady) {
                    console.log('✅ Imagen confirmada, actualizando navbar...');
                    try {
                        if (window.NavBarComponentInstance && typeof window.NavBarComponentInstance.loadUserData === 'function') {
                            ...
                            console.warn('⚠️ NavBarComponentInstance no disponible');
                        }
                    } catch (err) {
                        console.error('❌ Error actualizando navbar:', err);
                    }
                } else {
                    console.warn('⚠️ No se pudo confirmar la imagen, actualizando navbar de todas formas...');
                    ...
                    try {
                        ...
                        console.error('❌ Error actualizando navbar (fallback):', err);
                    }
                }
            } else {
                console.warn('⚠️ No hay URL de imagen válida');
     ...
     * CRÍTICO: Solo carga foto del usuario desde su carpeta ID, NUNCA imagen por defecto
     */
    updateProfilePicture() {
        ...
        try {
            ...
            if (imageUrl) {
                ...
                console.log('🖼️ Actualizando foto de perfil:', finalUrl);
                ...
                profilePic.onerror = function() {
                    console.error('❌ Error al cargar imagen actualizada:', finalUrl);
                    this.src = '';
                    this.style.display = 'none';
                    this.style.visibility = 'hidden';
                };
    ...
    updateNavbarAvatar() {
        try {
            ...
            if (!navbarProfilePic) {
                console.warn('⚠️ navbarProfilePic no encontrado');
                return;
            }
            ...
            console.log('🖼️ Actualizando avatar en navbar:', finalUrl);
            navbarProfilePic.src = finalUrl;
            
        } catch (err) {
            console.error('❌ Error en updateNavbarAvatar():', err);
        }
    }
