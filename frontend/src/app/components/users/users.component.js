...
class UsersComponent {
    ...
    async init() {
        try {
            console.log('🔄 Inicializando UsersComponent...');
            ...
            // Guardar el rol en la instancia para usarlo en otros métodos
            this.userRole = userRole;
            this.isAdmin = isAdmin;
            this.isPremium = isPremium;
            
            console.log('✅ Permisos validados - isAdmin:', isAdmin, 'isPremium:', isPremium);
            ...
            // Guardar instancia global para acceso desde métodos estáticos
            window.UsersComponentInstance = this;
            
            console.log('✅ UsersComponent inicializado exitosamente');
            ...
            if (container) {
                container.innerHTML = `
                    <div class="alert alert-danger m-5">
                        ...
                        <p><strong>Detalle:</strong> ${error.message || 'No se pudo cargar la lista de usuarios'}</p>
                        ${error.backendMessage ? `<p class="mt-2"><small>Backend: ${error.backendMessage}</small></p>` : ''}
    ...
    async loadUsers() {
        try {
            ...
            // Verificar que el JWT exista antes de hacer la petición
            const jwtToken = AuthService.getJwtToken();
            if (!jwtToken) {
                console.warn('⚠️ ADVERTENCIA: No hay JWT token, posiblemente sesión expirada');
            } else {
                console.log('✅ JWT token disponible:', jwtToken.substring(0, 20) + '...');
            }
            ...
            if (response.success) {
                ...
                // Si no hay usuarios, advertencia adicional
                if (this.users.length === 0 && this.totalItems === 0) {
                    console.warn('⚠️ La base de datos no contiene usuarios');
                }
            } else {
    ...
    renderEmpty() {
        return `
            <div class="text-center py-5">
                ...
                <p class="text-muted mb-3">No se encontraron otros usuarios en el sistema. (Solo se muestran usuarios diferentes al tuyo)</p>
                <div class="alert alert-info mt-4">
                    <small><i class="fas fa-info-circle me-2"></i>Información: Tu usuario está excluido automáticamente de esta lista.</small>
                </div>
            </div>
        `;
    }
    ...
    attachTableListeners() {
        ...
        document.querySelectorAll('.view-user-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                console.log(`👁️ Clic en "Ver" usuario: userId=${userId}`);
                
                if (userId) {
                    const url = '/users/' + userId;
                    console.log(`🔗 Navegando a: ${url}`);
                    App.getInstance().navigateTo(url);
                } else {
                    console.error('❌ userId no disponible en dataset');
                }
            });
        });
        ...
        document.querySelectorAll('.delete-user-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                console.log(`🗑️ Clic en "Eliminar" usuario ID: ${userId}, Nick: ${userNick}`);
    ...
    getRoleBadgeClass(role) {
        // Mapear roles a clases Bootstrap
        const roleBadgeMap = {
            'ADMIN': 'bg-danger',
        ...
        // Si el rol viene en mayúsculas sin espacios, buscar directamente
        const normalizedRole = (role || '').toUpperCase().trim();
        return roleBadgeMap[normalizedRole] || 'bg-secondary';
    }
