...
this.selector = '#router-outlet';
    ...
    async init(userId) {
        try {
            console.log(`📋 Inicializando UserDetailsComponent para usuario ID: ${userId}`);
            ...
            console.log(`🔐 Rol actual: ${userRole} (Usuario: ${currentUser?.email})`);
            ...
            console.log(`✅ Permisos validados. Cargando usuario ID: ${this.userId}`);
            this.show('Cargando...');
            await this.loadUser();
            
            console.log(`📤 Usuario cargado: ${this.user.nick} (ID: ${this.user.id})`);
            this.show(this.render());
            this.setupListeners();
        } catch (e) {
            console.error('❌ Error en UserDetailsComponent.init():', e);
    ...
    async loadUser() {
        console.log(`🔄 Cargando usuario ID: ${this.userId}`);
        ...
        // Extraer datos - puede estar en res.data o res.user
        ...
        // Validación crucial: verificar que el ID coincida
        if (this.user.id != this.userId) {
            console.error(`⚠️ INCONSISTENCIA: Solicitado ID ${this.userId} pero recibido ID ${this.user.id}`);
            throw new Error(`Error: Se recibió usuario ID ${this.user.id} pero se solicitó ID ${this.userId}`);
        }
        
        console.log(`✅ Usuario validado: ID=${this.user.id}, Nick=${this.user.nick}, Email=${this.user.email}`);
    }
