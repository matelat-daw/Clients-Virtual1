...
class CustomerService {
     ...
     * @returns {Promise<Object>}
     */
    static async searchByFirstName(firstName, page = 0, size = 10) {
        try {
            const url = `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.CUSTOMER_SEARCH_FIRSTNAME.replace(':firstName', encodeURIComponent(firstName))}?page=${page}&size=${size}`;
            const response = await Utils.makeRequest('GET', url);
            return response;
        } catch (error) {
            console.error('Error searching customers by firstName:', error);
            throw error;
        }
    }
     ...
     * @returns {Promise<Object>}
     */
    static async searchByLastName(lastName, page = 0, size = 10) {
        try {
            const url = `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.CUSTOMER_SEARCH_LASTNAME.replace(':lastName', encodeURIComponent(lastName))}?page=${page}&size=${size}`;
            const response = await Utils.makeRequest('GET', url);
            return response;
        } catch (error) {
            console.error('Error searching customers by lastName:', error);
            throw error;
        }
    }
