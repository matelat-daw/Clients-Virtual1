...
const Utils = {
    ...
    /**
     * Cache para peticiones GET con persistencia en SessionStorage
     */
    _cache: {
        get: (key) => {
            try {
                ...
                if (Date.now() - parsed.timestamp > Utils._cacheTTL) {
        ...
        set: (key, data) => {
            try {
                ...
                sessionStorage.setItem(`cache_${key}`, JSON.stringify(val));
            } catch (e) {}
        },
        clear: () => {
    ...
    /**
     * Tiempo de vida del cache en milisegundos (2 minutos para mayor frescura)
     */
    _cacheTTL: 2 * 60 * 1000,

    /**
     * Realiza una llamada HTTP con cache inteligente para GET
     ...
     * @returns {Promise}
     */
    makeRequest: async (method, url, data = null, useCache = false, timeout = 3000) => {
        const cacheKey = btoa(url).substring(0, 32);
        
        // Solo aplicar cache para GET
        if (method.toUpperCase() === 'GET' && useCache) {
            const cachedData = Utils._cache.get(cacheKey);
            if (cachedData) {
        ...
        // CONTROL DE TIEMPO: Si la API no responde en 'timeout' ms, abortamos.
        const controller = new AbortController();
        const timeoutId = setTimeout(() => {
            controller.abort();
            console.warn(`🛑 Petición abortada por timeout (${timeout}ms): ${url}`);
        }, timeout);
        ...
        try {
            ...
            // Inyectar token si existe - CON MEJOR LOGGING
            if (typeof AuthService !== 'undefined') {
                if (typeof AuthService.getAuthorizationHeader === 'function') {
                    ...
                    if (authHeader && authHeader.Authorization) {
                        ...
                        console.debug(`🔐 JWT inyectado en Authorization header para: ${method} ${url}`);
                    } else {
                        console.warn(`⚠️ No hay JWT disponible para: ${method} ${url}`);
                    }
                }
            } else {
                console.warn('⚠️ AuthService no está disponible');
            }
            ...
            console.debug(`📤 Enviando ${method} a ${url}`, options.headers);
            
            const response = await fetch(url, options);
            clearTimeout(timeoutId); // Limpiar el timer si la respuesta llega a tiempo
            
            console.debug(`📥 Respuesta recibida: ${response.status} ${response.statusText}`);
            ...
            if (method.toUpperCase() === 'GET' && useCache) {
                Utils._cache.set(cacheKey, responseData);
            }
            ...
            if (error.name === 'AbortError') {
                throw new Error('TIMEOUT: El servidor tarda demasiado en responder. Revisa la base de datos.');
            }
