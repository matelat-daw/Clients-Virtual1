    ...
    private ResponseEntity<Resource> serveResource(Resource resource, Path path) throws Exception {
        ...
        try {
            return ResponseEntity.ok()
                    ...
                    // Agregar headers de caché para reduce traffic
                    .header(HttpHeaders.CACHE_CONTROL, "public, max-age=86400") // Cache por 24 horas
                    .header(HttpHeaders.ETAG, "\"" + path.getFileName() + "-" + Files.getLastModifiedTime(path).toMillis() + "\"")
                    .body(resource);
        } catch (Exception ex) {
            // Si ocurre error escribiendo (cliente desconectado), loguear como WARN
            if (ex.getMessage() != null && (ex.getMessage().contains("Se ha anulado") || 
                ex.getMessage().contains("Connection reset") || 
                ex.getMessage().contains("Broken pipe"))) {
                log.warn("⚠️ Cliente desconectado mientras se servía imagen: {}", path.getFileName());
            } else {
                log.error("❌ Error sirviendo imagen {}: {}", path.getFileName(), ex.getMessage());
            }
            throw ex;
        }
    }
