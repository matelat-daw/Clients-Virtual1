...
public class ImageService {
    ...
    public Path ensureUserImageDirectory(Long userId) throws IOException {
        log.debug("🔧 Resolviendo ruta base de uploads...");
        Path uploadPath = resolveUploadBasePath();
        log.info("📂 Ruta base de uploads: {}", uploadPath.toAbsolutePath());
        
        log.debug("🔧 Creando/verificando directorio base...");
        Files.createDirectories(uploadPath);
        log.info("✅ Directorio base existe: {}", uploadPath.toAbsolutePath());

        Path userPath = uploadPath.resolve(String.valueOf(userId));
        log.info("🔧 Creando/verificando carpeta del usuario: {}", userPath.toAbsolutePath());
        Files.createDirectories(userPath);
        log.info("✅ Carpeta del usuario lista: {} (ID: {})", userPath.toAbsolutePath(), userId);
    ...
    public String saveProfileImage(MultipartFile file, Long userId) throws Exception {
        ...
        try {
            ...
            log.info("🔧 Asegurando que existe directorio del usuario {}", userId);
            ...
            log.info("📝 Ruta de archivo a guardar: {}", filePath.toAbsolutePath());

            // Guardar archivo (sobrescribirá si ya existe)
            log.info("💾 Escribiendo {} bytes en archivo...", file.getSize());
            ...
            if (Files.exists(filePath)) {
                ...
                log.info("✅✅ VERIFICACIÓN: Archivo existe y contiene {} bytes", fileSize);
            } else {
                log.error("❌❌ ERROR: Archivo NO existe después de write: {}", filePath.toAbsolutePath());
                throw new IOException("El archivo no se guardó correctamente en: " + filePath.toAbsolutePath());
            }
