...
public class GlobalExceptionHandler {
    ...
    /**
     * Maneja cuando el cliente se desconecta durante una transacción asincrona
     * NO es un error del servidor, es normal que ocurra cuando el usuario navega o cierra el navegador
     */
    @ExceptionHandler(AsyncRequestNotUsableException.class)
    public ResponseEntity<Void> handleAsyncRequestNotUsable(AsyncRequestNotUsableException ex) {
        // NO loguear como ERROR - es solo un cliente que se desconectó
        // Loguear como WARN para auditoría, pero sin el stack trace
        String clientMessage = ex.getMessage() != null ? ex.getMessage() : "Conexión de cliente perdida";
        
        // Extraer el causa raíz si existe
        Throwable cause = ex.getCause();
        String causeMsg = (cause != null && cause.getMessage() != null) ? cause.getMessage() : "IOException";
        
        log.warn("⚠️ [ClientDisconnect] {}: {}", causeMsg, clientMessage);
        
        // No intentar escribir respuesta - el cliente ya se fue
        // Retornar null para que Spring no intente escribir nada
        return null;
    }

    /**
     * Maneja IOExceptions generales de conexión (cliente desconectado)
     * Puede ocurrir cuando el cliente cierra la conexión abruptamente
     */
    @ExceptionHandler(IOException.class)
    public ResponseEntity<Void> handleIOException(IOException ex) {
        String message = ex.getMessage() != null ? ex.getMessage() : "Error de entrada/salida";
        
        // Detectar si es una desconexión de cliente
        if (message.contains("Se ha anulado una conexión") || 
            message.contains("Connection reset") ||
            message.contains("Broken pipe") ||
            message.contains("ClientAbort")) {
            // Desconexión de cliente - solo WARN sin stack trace
            log.warn("⚠️ [ClientDisconnect] Cliente desconectado abruptamente: {}", message);
        } else {
            // Otro tipo de IO error - loguear como ERROR
            log.error("❌ Error de I/O inesperado: {}", message, ex);
        }
        
        // No intentar escribir respuesta
        return null;
    }

    /**
     * Maneja excepciones de tiempo de espera de cliente
     */
    @ExceptionHandler(java.util.concurrent.TimeoutException.class)
    public ResponseEntity<ApiResponse<Void>> handleTimeoutException(java.util.concurrent.TimeoutException ex) {
        log.warn("⏱️ [Timeout] La solicitud tardó demasiado: {}", ex.getMessage());
        return ApiResponseBuilder.error(HttpStatus.REQUEST_TIMEOUT, "La solicitud tardó demasiado en procesar");
    }
